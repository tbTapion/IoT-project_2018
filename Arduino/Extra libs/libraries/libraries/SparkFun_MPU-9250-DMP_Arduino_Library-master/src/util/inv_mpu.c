...
// DS 2018 PROGMEM. Moved here from inv_mpu_motion_driver.c
//
#define DMP_CODE_SIZE           (3062)

#define PROGMEM   ICACHE_RODATA_ATTR  // DS - For ESP

int min(int a, int b) {  // DS - 2019
   if (a < b) {
      return a ;
   } else {
      return b ;
...
static const unsigned short sStartAddress = 0x0400;  // DS 2018.. MOVED HERE
#define DMP_SAMPLE_RATE     (200)  // DS 2018.. MOVED HERE
...
int mpu_load_firmware(void)  // DS 2018... All data now in PROGMEM in this file...
{
    unsigned short ii;
...
#define LOAD_CHUNK  (16)
    ...
    unsigned char buffer[LOAD_CHUNK];  // DS 2018 - buffer in mem from progmem

    if (st.chip_cfg.dmp_loaded)
        /* DMP should only be loaded once. */
        return -1;

   // if (!firmware)   // DS 2018. Is now local...
   //     return -1;
   
    int length = DMP_CODE_SIZE;
    ...
    for (ii = 0; ii < length; ii += this_write) {
       ...
       // DS 2018.. Code was:       
        ...
        memcpy_P(buffer, &dmp_memory[ii],this_write);   // DS 2018: Copy one chunck from PROGMEM to buffer.
        
        if (mpu_write_mem(ii, this_write, (unsigned char*)&buffer[0]))  // DS 2018...  Copy from buffer to IMU..
             return -1;            
        if (mpu_read_mem(ii, this_write, cur))  // DS 2018... Read data back from IMU..
            return -1;
        if (memcmp(buffer,cur, this_write))  // DS 2018... Compare with data in buffer...
            return -2;
 ...
 //   i2c_write(st.hw->addr, st.reg->prgm_start_h, 2, tmp);
    ...
    st.chip_cfg.dmp_sample_rate = DMP_SAMPLE_RATE;
    return 0;
}
